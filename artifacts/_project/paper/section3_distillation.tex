...
The SVD analysis (\S\ref{sec:svd}) reveals a geometric gap between RL and SFT weight changes, but geometry alone does not establish a causal link to distillation failure. In this section, we test whether preference-based learning (DPO) can close the gap, uncover a truncation confound that initially masked the true effect size, and examine behavioral evidence for why the student fails.
